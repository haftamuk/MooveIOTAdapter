...
function Server(opts, callback) {
  ...
  this.setAdapter = function (adapter) {
    ...
    logger.debug(`Adapter set to: ${adapter.model_name || 'unknown'}`);
  };
  ...
  this.init = function (cb) {
    ...
    logger.debug(`Server init called, debug mode: ${this.opts.debug}`);
  ...
  this.setDebug = function (val) {
    this.debug = (val === true);
    logger.debug(`Debug mode set to: ${this.debug}`);
  };
  ...
  this.init(function () {
    _this.server = net.createServer(function (connection) {
      ...
      // Determine protocol identifier for the device
      const adapter = _this.getAdapter();
      let protocol = null;
      if (adapter.protocol === 'JT808') {
        protocol = 'ut04s';
      } else if (adapter.protocol === 'GT06N') {
        protocol = 'gt06';
      } else {
        protocol = 'unknown';
      }

      const device = new Device(protocol, _this.getAdapter(), connection, _this);
      ...
      connection.on('end', function () {
        ...
        logger.debug(`Connection ended for device ${device.getUID() || 'unknown'}`);
      });
      ...
      logger.debug(`Connection fully established for device ${device.getUID() || 'unknown'}`);
    });
  ...
  this.find_device = function (deviceId) {
    logger.debug(`find_device called for ID: ${deviceId}`);
    for (const conn of this.devices) {
      if (conn.device.uid === deviceId) {
        logger.debug(`Device ${deviceId} found`);
        return conn.device;
      }
    }
    logger.debug(`Device ${deviceId} not found`);
    return false;
  };
  ...
  this.send_to = function (deviceId, msg) {
    logger.debug(`send_to called for device ${deviceId}`);
    const dev = this.find_device(deviceId);
    if (dev) dev.send(msg);
    else logger.warn(`send_to: device ${deviceId} not found`);
  };
