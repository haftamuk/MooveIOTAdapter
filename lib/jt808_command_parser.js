...
/**
 * Classify a JT808 packet based on its command ID.
 * @param {string} cmd - 4‑character hex command ID (e.g., '8103')
 * @returns {{ type: string, commandName: string }}
 */
function classifyJT808Packet(cmd) {
  // Platform → Terminal responses (less relevant for our proxy)
  const responseCommands = new Set([
    '8001', // Platform general response
    '8100', // Terminal registration response
    '8103', // Actually a command, but we treat it separately
    // Add more response IDs if needed
  ]);
  // Common command IDs (platform → terminal commands)
  const commandNames = {
  ...
  const isResponse = responseCommands.has(cmd);
  const commandName = commandNames[cmd] || `Unknown (0x${cmd})`;

  return {
    type: isResponse ? 'response' : 'command',
    commandName,
  };
}
...
function parseJT808Packet(buffer) {
  try {
    ...
    const classification = classifyJT808Packet(cmd);
