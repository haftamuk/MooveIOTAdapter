...
const { unescapeJT808, escapeJT808 } = require('./functions');
...
function Device(adapter, connection, gpsServer) {
  ...
  // [NEW] Count consecutive invalid messages
  this.invalidMessageCount = 0;
  // [NEW] Threshold – can be overridden via environment
  ...
  this.on('data', function (data) {
    ...
    while (startIdx !== -1) {
      ...
      // -------------------------------------------------------------
      // 1. Remove start/end markers and unescape the inner packet
      // -------------------------------------------------------------
      const innerHex = msgHex.slice(startMarker.length, msgHex.length - endMarker.length);
      const innerBuf = Buffer.from(innerHex, 'hex');
      const unescapedBuf = unescapeJT808(innerBuf);
      const unescapedHex = unescapedBuf.toString('hex').toUpperCase();

      // Reconstruct full message with markers? Not needed; adapter expects unescaped inner buffer.
      const msgParts = _this.adapter.parse_data(unescapedBuf); // <-- now unescaped

      if (msgParts === false) {
        logger.warn(`Invalid message discarded (after unescape): ${msgHex}`);
        // [NEW] Increment invalid counter and check threshold
        ...
        if (_this.invalidMessageCount >= _this.maxInvalidMessages) {
          ...
          // break out of loop – connection is gone
          break;
        }
      } else {
        // [NEW] Valid message resets counter
        _this.invalidMessageCount = 0;
  ...
  this.make_action = function (action, msgParts) {
    // If this is an incomplete subpackage, ignore it
  ...
  this.send = function (msg) {
    ...
    // Escape the buffer (except start/end markers – we add them later)
    const escaped = escapeJT808(buffer);
