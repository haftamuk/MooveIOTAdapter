...
const { isDebugDevice } = require('./deviceConfig');

util.inherits(Device, EventEmitter);

function Device(protocol, adapter, connection, gpsServer) {
  EventEmitter.call(this);

  const _this = this;

  this.protocol = protocol;     // 'ut04s' or 'gt06'
  ...
  this.setUID = function (uid) {
    ...
    if (isDebugDevice(this.protocol, uid)) {
