...
// No direct import of unescape/escape here; adapters handle it.
...
function Device(adapter, connection, gpsServer) {
  ...
  // Count consecutive invalid messages
  ...
  this.on('data', function (data) {
    ...
    while (startIdx !== -1) {
      ...
      // Convert the whole message (including markers) to a Buffer and pass to adapter
...
this.ping = function (msgParts) {
 ...
 this.receive_alarm = function (msgParts) {
  ...
  this.send = function (msg) {
    ...
    // Wrap with start and end markers – assume the adapter already escaped the content if needed.
